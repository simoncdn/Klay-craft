...
<template>
	<div :class="cn('flexBetween paddingX py-2')">
		<span>KC</span>

		<nav>
			<ul :class="cn('flex space-x-8')">
				<li v-for="route in routes" :class="cn(
					'cursor-pointer',
					...
					<span v-if="isSelected(route.path)" :class="cn('w-1.5 h-1.5', 'rounded-full', 'bg-carbon')"></span>
				</li>
			</ul>
		</nav>
	</div>
