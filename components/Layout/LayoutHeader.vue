...
<template>
    <header :class="cn('z-20', 'top-0', 'paddingX py-2', 'flexBetween items-center')" :style="{ color: activeColor }">
        <div>
                    ...
                    <div :class="cn('gap-4', 'flexCol')">
                        <nav>
                            <ul :class="cn('gap-8', 'flexCol items-center')">
                                <li v-for="route in routes" :key="route.path" :class="cn('text-2xl uppercase')">
        ...
        <div :class="cn('gap-16', 'flexCenter items-center')">
            <nav :class="cn('hidden lg:flex')">
                <ul :class="cn('flex space-x-8')">
                    <li v-for="route in routes" :key="route.path" :class="cn(
        ...
        ">
                        <NuxtLink :to="route.path" class="transition-all duration-300 hover:opacity-70">{{ route.name }}
                        </NuxtLink>
                        <span v-if="isSelected(route.path)" :class="cn('absolute -bottom-4', 'h-1.5 w-1.5', 'rounded-full', 'mx-auto')
        " :style="{ backgroundColor: activeColor }"></span>
                    </li>
                </ul>
            </nav>
